		...
		try {
			for (ChannelGroup channelGroup : groups) {
				...
				for (Fav fav : channelGroup.getFavs()) {
					db.execSQL("update " + ChannelTbl.TABLE_NAME + " set " + ChannelTbl.FAV_POSITION + " = " + fav.position + ", " + ChannelTbl.FAV_GROUP_ID + " = " + groupId + ", " + ChannelTbl.FLAGS + " = " + ChannelTbl.FLAGS + " | " + Channel.FLAG_FAV + " where " + ChannelTbl._ID + " = '" + fav.id + "';");
				}
			}
			db.setTransactionSuccessful();
		} catch (Exception e) {
